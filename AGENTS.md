...
`scripts/probe_api_bonuses.py`. The fourteen local files in `backend/samples/demo/` are the same images exposed by `/api/samples`; `scripts/validate_demo_samples.py` validates this single corpus. Runtime qualification uses separate verification inputs.
