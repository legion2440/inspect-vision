...
- `backend/routes/sample_catalog.py` defines the operator-facing Samples catalog:
  eight MVTec Bayes-PFL examples plus three steel and three concrete specialist
  examples. `/api/samples` exposes only that catalog; inspecting an entry reuses
  the ordinary persisted `/api/inspect` path.
- `backend/samples/demo-samples.json` plus `backend/samples/demo/` are a separate
  twelve-image VisA audit/demo corpus. `sourceGroundTruth` is dataset truth and
  retained `modelObservation` is historical evidence only. This corpus satisfies
  demo/evidence checks but must never replace or leak into the operator Samples page.
- Runtime qualification inputs are separate from both operator showcase and demo
...
positives must be recorded without relabeling source truth. The operator Samples
page is intentionally separate and is defined by `backend/routes/sample_catalog.py`.
