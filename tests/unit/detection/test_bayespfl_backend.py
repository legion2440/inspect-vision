...
def test_config_rejects_invalid_candidate_threshold() -> None:
...
def test_candidate_checkpoint_uses_weights_only() -> None:
    calls = []
    ...
    checkpoint = _load_candidate_checkpoint(Path(__file__), FakeTorch)
...
def test_backend_owned_geometry_restores_candidate_boxes_once(tmp_path: Path) -> None:
    ...
    assert result.detections[0].xyxy == pytest.approx((80.0, 20.0, 160.0, 50.0))
    assert result.detections[0].class_name == "anomaly"


def test_candidate_requires_product_name(tmp_path: Path) -> None:
