...
`PARTIAL` still needs a current end-to-end runtime confirmation, and `PLANNED`
has no implementation claim.
...
| Full application starts without errors | PARTIAL | FastAPI composition and frontend build paths are implemented; the current Bayes-PFL default still needs the final local runtime run with installed artifacts | `python scripts/validate.py`, then `python scripts/probe_models.py --device cpu` |
...
| History GET/DELETE/clear | PASS | List/detail/delete/clear use persisted metadata and owned media cleanup | API/storage tests and retained records |
...
| Real model inference | PARTIAL | Specialists and earlier model milestones have committed runtime records; the new Bayes-PFL default implementation is wired but its current production runtime probe is intentionally left for the final local run | `python scripts/probe_models.py --device cpu` |
...
current. The final local run should validate the current tree first, then run the
model probe with the installed Bayes-PFL artifacts.
