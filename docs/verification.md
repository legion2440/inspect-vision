...
| Full application starts without errors | PARTIAL | The previous local run is retained historically; the current model-selection/category/showcase/device update still requires the normal post-change validation and runtime qualification | `python scripts/validate.py`; then `python scripts/probe_models.py --device auto` |
...
| Real model inference | PARTIAL | The retained CPU record covers the previous four-model exposure state. It is immutable historical evidence; a fresh three-model production-service probe on the selected current device is pending | `docs/evidence/models/model-registry-acceptance.json`; `docs/project-status.json` |
