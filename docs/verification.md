...
| Comprehensive root README | PASS | Setup, model installation, selection rationale, API, samples, structure, and validation are documented | `README.md` |
| Full application starts without errors | PARTIAL | The previous local run is retained historically; the current model-selection/category/showcase update still requires the normal post-change validation and runtime qualification | `python scripts/validate.py`; then `python scripts/probe_models.py --device cpu` |
...
| Real model inference | PARTIAL | The retained CPU record covers the previous four-model exposure state. It is immutable historical evidence; a fresh three-model production-service probe is pending | `docs/evidence/models/model-registry-acceptance.json`; `docs/project-status.json` |
...
Historical runtime bundles are immutable and validated against the source
snapshots they recorded. When current detector source differs from the latest
runtime bundle, repository validation requires an explicit
`runtime-requalification-pending` project status instead of rewriting that old
record. A new runtime bundle becomes current only after the production-service
probe is actually run.
