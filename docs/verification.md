...
| Guided product context | PASS | Bayes-PFL category context is normalized, length/word/character validated, accepted by inspect/stream, and included in the runtime cache key; the UI exposes curated values through an editable combobox while preserving custom input | runtime/API/frontend tests |
...
`auto` resolving to CUDA on an NVIDIA GeForce RTX 4080 Laptop GPU. The current
UI/showcase change does not alter detector-bound source, so that runtime bundle
remains current. Earlier runtime bundles remain recoverable from Git history and
stay bound to the source snapshots they recorded. A future detector-source
change must again set `runtime-requalification-pending` until a new
production-service probe is run.
